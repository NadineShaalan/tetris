...
;;; English: If 10 blocks have the same y coord then the row is full
;;; How do we test for that?

;;; bset Number -> Number
;;; How many blocks are in the list with the specified y coord
(define (how-many bs num)
  (local [(define (is-at? x) (= (block-y x) num))]
    (length (filter is-at? bs))))

(check-expect (how-many (list (make-block 1 0 'green)
                              (make-block 2 0 'green)
                              (make-block 2 2 'green)) 0)
              2)
;;; Bs -> Boolean
(define (row-full? bs row)
  (= (how-many bs row) 10))
(check-expect (row-full? (list (make-block 1 0 'blue)
                              (make-block 1 0 'blue)
                              (make-block 1 0 'blue)
                              (make-block 1 0 'blue)
                              (make-block 1 0 'blue)
